...
func (w *World) tickLoop() {
	...
	for range ticker.C {
		...
		if w.tickRate != currentTps {
			currentTps = w.tickRate
			ticker.Reset(time.Second / time.Duration(currentTps))
		}
		w.Tick()
	}
}

func (w *World) chunkLoop() {
...
func (w *World) GetTickCount() uint {
	return w.tickCount
}
...
func (w *World) Tick() {
	...
	w.tickScheduler(currentTick)

	w.TPS.TotalTicks++
	tickstart := time.Now()

	animate.Tick(currentTick)

	ts1 := time.Since(tickstart)
	fmt.Println("Animate Tick took", ts1, currentTick)

	w.TPS.AnimateTime += ts1
	tickstart = time.Now()

	hpcworld.Tick()

	ts2 := time.Since(tickstart)
	fmt.Println("Tick took", ts2, currentTick)

	w.TPS.HPCTickTime += ts2
...
func (w *World) subtickChunkLoad() {
	for c, p := range w.players {
		x := int32(math.Floor(p.Position[0] / 16.0))
		y := int32(math.Floor(p.Position[1] / 16.0))
		z := int32(math.Floor(p.Position[2] / 16.0))
		if newChunkPos := [3]int32{x, y, z}; newChunkPos != p.ChunkPos {
			p.ChunkPos = newChunkPos
			c.SendSetChunkCacheCenter([2]int32{x, z})
			// w.broadcastSystemMessage(chat.Text(fmt.Sprintf("Player %s moved to chunk %v", p.Name, newChunkPos)))
		}
	}
	...
	for viewer, loader := range w.loaders {
		...
		for _, pos := range loader.loadQueue {
			...
			lc.Lock()
			lc.refresh()
			viewer.ViewChunkLoad(pos, lc)
			lc.Unlock()
		}

		for _, pos := range loader.unloadQueue {
			// fmt.Println("unloading chunk", pos)
			if lc, ok := w.chunks[pos]; ok {
				lc.RemoveViewer(viewer)
				viewer.ViewChunkUnload([2]int32{pos[1], pos[0]})
			}
			delete(loader.loaded, pos)
		}

		// var loadedstr []string
		// for pos := range loader.loaded {
		// 	loadedstr = append(loadedstr, fmt.Sprintf(" (%d,%d)", pos[0], pos[1]))
		// }

		// sort.Slice(loadedstr, func(i, j int) bool {
		// 	return loadedstr[i] < loadedstr[j]
		// })

		// fmt.Printf("loa/ded chunks:%s\n", strings.Join(loadedstr, ","))
