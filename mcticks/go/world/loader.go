...
func (l *loader) calcLoadingQueue() {
	...
	for _, v := range loadList[:radiusIdx[r]] {
		var rpos [2]int32
		rpos[0], rpos[1] = pos[0]+v[0], pos[1]+v[1]

		if _, ok := l.loaded[rpos]; !ok {
			l.loadQueue = append(l.loadQueue, rpos)
		}
	}
}
...
func (l *loader) calcUnusedChunks() {
	...
	for chunk := range l.loaded {
		r := l.chunkRadius()
		if cheby2i(chunk[0]-player[0], chunk[1]-player[1]) > r {
...
func cheby2i(dx, dz int32) int32 {
	ax, az := dx, dz
	if ax < 0 {
		ax = -ax
	}
